...
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10000,
  message: "Too many requests created from this IP, please try again after a 15-minute interval",
});
app.use("/api", limiter);
...
app.get('/get-recommendations', async (req, res, next) => {
  try {
    ...
    const data = await Promise.all(response.data.map(async (course) => {
      return await Course.findOne({ title: course });
    }));

    res.json(data);
  } catch (error) {
    next(new ApiError(`Failed to fetch recommendations: ${error.message}`, 500));
  }
});
